...
async def candidate_user(db_session: AsyncSession, test_roles: dict) -> User:
    ...
    user_role = UserRoleModel(
        user_id=user.id,
        role_id=test_roles[UserRole.CANDIDATE.value].id
    )
...
@pytest.fixture
async def recruiter_user(db_session: AsyncSession, recruiter_company: Company, test_roles: dict) -> User:
    ...
    user_role = UserRoleModel(
        user_id=user.id,
        role_id=test_roles[UserRole.RECRUITER.value].id
    )
    db_session.add(user_role)
    await db_session.commit()
...
@pytest.fixture
async def employer_user(db_session: AsyncSession, employer_company: Company, test_roles: dict) -> User:
    ...
    user_role = UserRoleModel(
        user_id=user.id,
        role_id=test_roles[UserRole.EMPLOYER.value].id
    )
    db_session.add(user_role)
    await db_session.commit()
...
class TestMessagingAPI:
    ...
    async def test_create_conversation_success(
        ...
        # Create conversation
        response = await client.post("/api/messaging/conversations", 
                                   headers=headers,
                                   json={
                                       "participant_ids": [recruiter_user.id],
    ...
    async def test_create_conversation_forbidden(
        ...
        response = await client.post("/api/messaging/conversations",
                                   headers=headers,
                                   json={
                                       "participant_ids": [employer_user.id]
                                   })
    ...
    async def test_send_message_success(
        ...
        candidate_user: User, recruiter_user: User
    ):
        ...
        # Send message
        response = await client.post(f"/api/messaging/conversations/{conversation.id}/messages",
                                   headers=headers,
                                   json={
    ...
    async def test_get_conversation_messages(
        ...
        # Get messages
        response = await client.get(f"/api/messaging/conversations/{conversation.id}/messages",
                                  headers=headers)
    ...
    async def test_file_upload_presign(
        ...
        # Request file upload
        with patch('app.services.storage_service.storage_service.get_presigned_upload_url') as mock_presign:
    ...
    async def test_search_participants(
        ...
        # Search participants
        response = await client.get("/api/messaging/participants/search?query=recruiter",
                                  headers=headers)
    ...
    async def test_mark_messages_read(
        ...
        message = await messaging_service.send_message(
            db_session, conversation.id, recruiter_user.id, "Test message from recruiter"
        )
...
class TestMessagingService:
    ...
    async def test_conversation_access_control(
        ...
        can_access_candidate, _ = await messaging_service.rules_service.can_access_conversation(
            db_session, candidate_user.id, conversation.id
        )
        assert can_access_candidate is True
        
        can_access_recruiter, _ = await messaging_service.rules_service.can_access_conversation(
            db_session, recruiter_user.id, conversation.id
        )
        assert can_access_recruiter is True
        
        # Test access for non-participant
        can_access_employer, reason = await messaging_service.rules_service.can_access_conversation(
