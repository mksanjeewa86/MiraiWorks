...
class TestJobEndpoints:
    ...
    async def test_create_job_success(
        self, client: AsyncClient, auth_headers: dict, job_data: dict
    ):
        """Test successful job creation with valid data."""
        with patch("app.crud.job.job.create_with_slug") as mock_create:
            mock_job = Job(
                **job_data, id=1, slug="senior-software-engineer", status="draft"
            )
            mock_create.return_value = mock_job

            response = await client.post(
                "/api/jobs/", json=job_data, headers=auth_headers
            )
    ...
    async def test_get_job_by_id_success(self, client: AsyncClient):
        ...
        with patch("app.crud.job.job.get") as mock_get, patch(
            "app.crud.job.job.increment_view_count"
        ) as mock_increment:
    ...
    async def test_get_job_by_slug_success(self, client: AsyncClient):
        ...
        with patch("app.crud.job.job.get_by_slug") as mock_get, patch(
            "app.crud.job.job.increment_view_count"
        ) as mock_increment:
            mock_job = Job(
                id=1,
    ...
    async def test_update_job_success(
        self, client: AsyncClient, auth_headers: dict, job_update_data: dict
    ):
        """Test successful job update."""
        with patch("app.crud.job.job.get") as mock_get, patch(
            "app.crud.job.job.update"
        ) as mock_update:
            ...
            response = await client.put(
                "/api/jobs/1", json=job_update_data, headers=auth_headers
            )
    ...
    @pytest.mark.asyncio
    async def test_get_company_jobs_success(
    ...
    async def test_update_job_status_success(
        ...
        with patch("app.crud.job.job.get") as mock_get, patch(
            ...
            response = await client.patch(
                "/api/jobs/1/status", json=status_data, headers=auth_headers
            )
    ...
    async def test_bulk_update_status_success(
        ...
        bulk_data = {"job_ids": [1, 2, 3], "status": "closed"}
        ...
        with patch("app.crud.job.job.bulk_update_status") as mock_bulk:
            ...
            response = await client.patch(
                "/api/jobs/bulk/status", json=bulk_data, headers=auth_headers
            )
    ...
    @pytest.mark.asyncio
    async def test_update_job_unauthorized(
        self, client: AsyncClient, job_update_data: dict
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_job_insufficient_permissions(
        self, client: AsyncClient, candidate_headers: dict, job_data: dict
    ):
        """Test job creation with insufficient permissions fails."""
        response = await client.post(
            "/api/jobs/", json=job_data, headers=candidate_headers
        )
    ...
    @pytest.mark.asyncio
    async def test_create_job_invalid_title(
        self, client: AsyncClient, auth_headers: dict
    ):
        ...
        response = await client.post(
            "/api/jobs/", json=invalid_data, headers=auth_headers
        )
    ...
    @pytest.mark.asyncio
    async def test_update_job_invalid_salary_range(
        self, client: AsyncClient, auth_headers: dict
    ):
        ...
        with patch("app.crud.job.job.get") as mock_get:
            ...
            response = await client.put(
                "/api/jobs/1", json=invalid_data, headers=auth_headers
            )
            assert response.status_code == 422
    ...
    async def test_update_job_not_found(
        ...
        with patch("app.crud.job.job.get") as mock_get:
            ...
            response = await client.put(
                "/api/jobs/999", json=job_update_data, headers=auth_headers
            )
            assert response.status_code == 404
            assert "Job not found" in response.json()["detail"]
    ...
    @pytest.mark.asyncio
    async def test_company_jobs_permission_check(
        self, client: AsyncClient, employer_headers: dict
    ):
