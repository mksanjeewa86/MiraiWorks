    ...
    return (
      <div className={clsx('flex flex-col space-y-2', { 'w-full': fullWidth })}>
        ...
        <div className="relative">
          ...
          {rightIcon && (
            <div className="absolute right-3 top-1/2 transform -translate-y-1/2" style={{ color: 'var(--text-muted)' }}>
              {rightIcon}
            </div>
          )}
        </div>
        ...
        {helperText && !error && (
          <p className="text-sm" style={{ color: 'var(--text-muted)' }}>{helperText}</p>
        )}
      </div>
    );
  }
