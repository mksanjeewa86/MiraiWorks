...
async def reset_all_passwords(db: AsyncSession):
    ...
    result = await db.execute(
        update(User).values(hashed_password=new_password_hash)
    )
...
async def create_basic_users_directly(db: AsyncSession):
    ...
    # Get company IDs
    techcorp_result = await db.execute(text("SELECT id FROM companies WHERE name = 'TechCorp Solutions'"))
    techcorp_id = techcorp_result.scalar()
    
    recruiters_result = await db.execute(text("SELECT id FROM companies WHERE name = 'Global Recruiters Inc'"))
    ...
    users_data = [
        ...
        ('recruiter@globalrecruiters.com', password_hash, 'Sarah', 'Wilson', recruiters_id, False),
    ...
    for email, hashed_pwd, first_name, last_name, company_id, is_admin in users_data:
        ...
        existing_user = await db.execute(text("SELECT id FROM users WHERE email = :email"), {"email": email})
...
async def main():
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            if existing_users:
                print(f"Found {len(existing_users)} existing users. Resetting passwords...")
