    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            position_applications_check = await db.execute(
                text(
                    "SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'position_applications'"
                )
            )
            ...
            job_applications_check = await db.execute(
                text(
                    "SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'job_applications'"
                )
            )
            ...
            results["tables_renamed"] = (
                results["positions_table_exists"]
                and results["position_applications_table_exists"]
                and not results["jobs_table_exists"]
                and not results["job_applications_table_exists"]
            )
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            applications_count = await db.execute(
                text("SELECT COUNT(*) FROM position_applications")
            )
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            results["indexes_updated"] = (
                len(results["new_indexes_exist"]) == len(new_indexes)
                and len(results["old_indexes_exist"]) == 0
            )
...
async def main():
    ...
    if table_results["tables_renamed"]:
        ...
        print(
            f"   - position_applications table exists: {table_results['position_applications_table_exists']}"
        )
        print(f"   - jobs table removed: {not table_results['jobs_table_exists']}")
        print(
            f"   - job_applications table removed: {not table_results['job_applications_table_exists']}"
        )
    else:
        ...
        print(
            f"   - job_applications table still exists: {table_results['job_applications_table_exists']}"
        )
        return False
    ...
    if data_results["data_preserved"] and data_results["foreign_keys_working"]:
        ...
        print(
            f"   - Position applications count: {data_results['position_applications_count']}"
        )
        print(
            f"   - Foreign key relationships working: {data_results['foreign_keys_working']}"
        )
    else:
    ...
    if index_results["indexes_updated"]:
        ...
        print(
            f"   - Old indexes removed: {len(index_results['old_indexes_exist']) == 0}"
        )
    else:
