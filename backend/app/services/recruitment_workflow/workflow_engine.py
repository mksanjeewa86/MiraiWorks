...
class WorkflowEngineService:
    ...
    async def assign_candidate(
        ...
        candidate_proc = await candidate_process.create(
    ...
    async def start_candidate_process(
        ...
        # Find the first node
        start_nodes = await process_node.get_start_nodes(
    ...
    async def _create_interview_for_execution(
        self, db: AsyncSession, execution: NodeExecution, node: ProcessNode
    ) -> None:
        ...
        # Link to execution
        await node_execution.link_interview(
    ...
    async def validate_process(
        ...
        flow_validation = await node_connection.validate_process_flow(
    ...
    async def get_process_analytics(
        ...
        # Basic statistics
        stats = await candidate_process.get_statistics_by_process(
            db, process_id=process_id
        )
        ...
        # Execution workload
        workload = await node_execution.get_workload_by_assignee(
            db, process_id=process_id
        )
    ...
    async def clone_process(
        ...
        source_process = await recruitment_process.get_with_nodes(
