...
down_revision: str | None = "enhance_resume_system_japanese"
...
def upgrade() -> None:
    ...
    op.add_column(
        "todos",
        sa.Column("todo_type", sa.String(20), nullable=False, server_default="regular"),
    )
    op.add_column(
        "todos",
        sa.Column(
            "publish_status", sa.String(20), nullable=False, server_default="published"
        ),
    )
    ...
    op.add_column(
        "todos", sa.Column("submitted_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column(
        "todos", sa.Column("reviewed_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column("todos", sa.Column("reviewed_by", sa.Integer(), nullable=True))

    # Add foreign key constraint for reviewed_by
    op.create_foreign_key(
        "fk_todos_reviewed_by",
        "todos",
        "users",
        ["reviewed_by"],
        ["id"],
