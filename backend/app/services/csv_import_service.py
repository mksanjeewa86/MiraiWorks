...
class CSVImportService:
    ...
    def validate_csv_data(self, csv_content: str) -> Tuple[bool, List[str], pd.DataFrame]:
            ...
            for index, row in df.iterrows():
                ...
                if pd.isna(row['role']) or not row['role']:
                    ...
                    try:
                        ...
                        row_errors.append(f"Invalid role. Must be one of: {', '.join(valid_roles)}")
                ...
                for bool_field in ['is_admin', 'require_2fa']:
                    if bool_field in df.columns and not pd.isna(row[bool_field]):
                        ...
                        if val not in ['true', 'false', '1', '0', 'yes', 'no']:
                            row_errors.append(f"{bool_field} must be true/false or 1/0 or yes/no")
        ...
        for index, row in df.iterrows():
            try:
                ...
                if existing_user.scalar_one_or_none():
                    failed_users.append(f"Row {index + 2}: User with email {row['email']} already exists")
                    continue
                ...
                if company_id:
                    ...
                    if not company_result.scalar_one_or_none():
                        failed_users.append(f"Row {index + 2}: Company ID {company_id} does not exist")
                        continue
            ...
            try:
                ...
                for user_info in created_users:
                    try:
                        ...
                        logger.error(f"Failed to send activation email to {user_info['email']}: {str(e)}")
