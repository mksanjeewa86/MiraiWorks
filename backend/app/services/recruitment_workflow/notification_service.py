...
class RecruitmentWorkflowNotificationService:
    ...
    async def notify_node_completed(
        ...
        if candidate_process.assigned_recruiter_id and execution.completed_by != candidate_process.assigned_recruiter_id:
            notifications.append({
                "user_id": candidate_process.assigned_recruiter_id,
                "type": "node_completed",
        ...
        if execution.feedback and candidate_process.candidate_id != execution.completed_by:
            notifications.append({
                "user_id": candidate_process.candidate_id,
    ...
    async def notify_process_completed(
        ...
        }.get(candidate_process.final_result, "Your application process has been completed.")
    ...
    async def notify_deadline_approaching(
        ...
        if node.node_type in ["todo", "assessment"] and candidate_process.candidate_id != execution.assigned_to:
            notifications.append({
                "user_id": candidate_process.candidate_id,
    ...
    async def notify_task_overdue(
        ...
        if (candidate_process.assigned_recruiter_id and
            candidate_process.assigned_recruiter_id != execution.assigned_to):
            notifications.append({
                "user_id": candidate_process.assigned_recruiter_id,
    ...
    async def notify_interview_scheduled(
        ...
        if execution.assigned_to and execution.assigned_to != candidate_process.candidate_id:
            notifications.append({
                "user_id": execution.assigned_to,
    ...
    async def _send_notifications(
        ...
        for notification in notifications:
            print(f"NOTIFICATION: {notification['type']} to user {notification['user_id']}: {notification['message']}")
