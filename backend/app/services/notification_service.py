...
class NotificationService:
    ...
    async def handle_new_message_notifications(
        ...
        conversation_key = f"{min(sender_id, recipient_id)}_{max(sender_id, recipient_id)}"
        now = datetime.utcnow()
        ...
        if conversation_key in self._email_debounce:
            ...
            if now - last_email_time < timedelta(minutes=5):
                logger.info(f"Email notification debounced for conversation {conversation_key}")
                return
