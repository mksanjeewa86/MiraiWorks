...
class TestResumeEndpointsAuthentication:
    ...
    async def test_update_resume_unauthorized(self, client: AsyncClient, test_resume: Resume):
        """Test updating resume without authentication fails."""
        response = await client.put(f"/api/resumes/{test_resume.id}", json={"title": "Updated"})
        assert response.status_code == 401

    async def test_delete_resume_unauthorized(self, client: AsyncClient, test_resume: Resume):
...
class TestResumeEndpointsValidAuthentication:
    ...
    async def test_get_resumes_success(self, client: AsyncClient, auth_headers: dict, test_resume: Resume):
    ...
    async def test_get_resume_by_id_success(self, client: AsyncClient, auth_headers: dict, test_resume: Resume):
    ...
    async def test_update_resume_success(self, client: AsyncClient, auth_headers: dict, test_resume: Resume):
        ...
        response = await client.put(f"/api/resumes/{test_resume.id}", json=update_data, headers=auth_headers)
        assert response.status_code == 200
    ...
    async def test_delete_resume_success(self, client: AsyncClient, auth_headers: dict, test_resume: Resume):
...
class TestEnumValidation:
    ...
    async def test_valid_visibility_values(self, client: AsyncClient, auth_headers: dict):
    ...
    async def test_invalid_visibility_values(self, client: AsyncClient, auth_headers: dict):
    ...
    async def test_valid_format_values(self, client: AsyncClient, auth_headers: dict):
        ...
        valid_formats = ["rirekisho", "shokumu_keirekisho", "international", "modern", "creative"]
...
class TestStatusTransitions:
    ...
    async def test_draft_to_published(self, client: AsyncClient, auth_headers: dict, test_resume: Resume):
    ...
    async def test_published_to_archived(self, client: AsyncClient, auth_headers: dict, test_resume: Resume):
...
class TestVisibilityAndPublicAccess:
    ...
    async def test_toggle_public_visibility(self, client: AsyncClient, auth_headers: dict, test_resume: Resume):
        """Test toggling public visibility."""
        response = await client.post(f"/api/resumes/{test_resume.id}/toggle-public", headers=auth_headers)
        assert response.status_code == 200
    ...
    async def test_update_public_settings(self, client: AsyncClient, auth_headers: dict, test_resume: Resume):
...
class TestErrorHandling:
    ...
    async def test_update_nonexistent_resume(self, client: AsyncClient, auth_headers: dict):
        """Test updating non-existent resume returns 404."""
        response = await client.put("/api/resumes/99999", json={"title": "New Title"}, headers=auth_headers)
        assert response.status_code == 404

    async def test_delete_nonexistent_resume(self, client: AsyncClient, auth_headers: dict):
        """Test deleting non-existent resume returns 404."""
        response = await client.delete("/api/resumes/99999", headers=auth_headers)
        assert response.status_code == 404

    async def test_create_resume_missing_required_fields(self, client: AsyncClient, auth_headers: dict):
        ...
        response = await client.post("/api/resumes/", json=incomplete_data, headers=auth_headers)
        assert response.status_code == 422

    async def test_create_resume_invalid_data_types(self, client: AsyncClient, auth_headers: dict):
        ...
        response = await client.post("/api/resumes/", json=invalid_data, headers=auth_headers)
        assert response.status_code == 422
...
class TestPagination:
    ...
    async def test_resumes_pagination(self, client: AsyncClient, auth_headers: dict):
        ...
        response = await client.get("/api/resumes/?limit=5&offset=0", headers=auth_headers)
        assert response.status_code == 200
...
class TestConcurrency:
    ...
    async def test_concurrent_resume_updates(self, client: AsyncClient, auth_headers: dict, test_resume: Resume):
