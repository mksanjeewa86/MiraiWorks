...
@pytest_asyncio.fixture
async def test_user(db_session: AsyncSession, test_company: Company, test_roles: dict[str, Role]) -> User:
    """Create test user."""
    user = User(
        email="test@test.com",
        first_name="Test",
    ...
    user_role = UserRole(
        user_id=user.id,
        role_id=test_roles["candidate"].id,
        scope="default"
    )
...
@pytest_asyncio.fixture
async def test_user2(db_session: AsyncSession, test_company: Company, test_roles: dict[str, Role]) -> User:
    ...
    user_role = UserRole(
        user_id=user.id,
        role_id=test_roles["recruiter"].id,
        scope="default"
    )
    db_session.add(user_role)
...
@pytest_asyncio.fixture
async def test_admin_user(db_session: AsyncSession, test_company: Company, test_roles: dict[str, Role]) -> User:
    ...
    user_role = UserRole(
        user_id=user.id,
        role_id=test_roles["company_admin"].id,
        scope="default"
    )
    db_session.add(user_role)
...
@pytest_asyncio.fixture
async def test_message(db_session: AsyncSession, test_user: User, test_user2: User) -> DirectMessage:
...
@pytest_asyncio.fixture
async def test_messages(db_session: AsyncSession, test_user: User, test_user2: User) -> list[DirectMessage]:
...
def mock_email_service():
    ...
    with pytest.mock.patch.object(email_service, 'send_email', new_callable=AsyncMock) as mock:
...
def mock_notification_service():
    ...
    with pytest.mock.patch.object(notification_service, 'create_notification', new_callable=AsyncMock) as mock:
        yield mock
