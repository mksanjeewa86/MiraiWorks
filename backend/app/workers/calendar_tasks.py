...
def sync_calendar_events_task(self, calendar_integration_id: int, force_full_sync: bool = False):
    """Periodic task to sync calendar events for a specific integration."""
    return run_async_task(_sync_calendar_events(calendar_integration_id, force_full_sync))
...
async def _sync_calendar_events(calendar_integration_id: int, force_full_sync: bool = False):
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            if not calendar_integration or not calendar_integration.is_active:
                logger.warning(f"Calendar integration {calendar_integration_id} not found or inactive")
            ...
            if calendar_integration.provider == "google":
                ...
                for event_data in events.get("items", []):
                    await _process_calendar_event(db, calendar_integration, event_data, "google")
                    synced_count += 1
                ...
                for event_data in events.get("value", []):
                    await _process_calendar_event(db, calendar_integration, event_data, "microsoft")
                    synced_count += 1
            ...
            logger.info(f"Synced {synced_count} events for calendar integration {calendar_integration_id}")
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            for integration in integrations:
                try:
                    result = await _sync_calendar_events(integration.id, force_full_sync=False)
                    total_synced += result.get("synced_events", 0)
                except Exception as e:
                    logger.error(f"Failed to sync integration {integration.id}: {str(e)}")
                    failed_syncs += 1
            
            logger.info(f"Bulk sync completed: {total_synced} events, {failed_syncs} failures")
            return {
                "status": "completed",
...
async def _check_interview_conflicts(interview_id: int):
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            conflicts = await interview_service.check_interview_conflicts(db, interview_id)
            
            if conflicts:
                logger.warning(f"Interview {interview_id} has {len(conflicts)} conflicts")
...
async def _cleanup_expired_calendar_tokens():
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            logger.info(f"Token cleanup: {refreshed_count} refreshed, {disabled_count} disabled")
            return {
                "status": "completed",
        ...
        try:
            ...
            for user_id in participant_ids:
                try:
                    ...
                    error_msg = f"Failed to sync to calendar for user {user_id}: {str(e)}"
