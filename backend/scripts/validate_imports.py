...
class ImportValidator:
    def __init__(self, project_root: str):
        ...
        self.optional_deps = {
            'vosk', 'speech_recognition', 'playwright', 'PyPDF2',
    ...
    def check_unused_imports(self, file_path: Path) -> List[str]:
        ...
        try:
            result = subprocess.run([
                ...
                str(file_path), '--select', 'F401', '--quiet'
    ...
    def check_import_sorting(self, file_path: Path) -> List[str]:
        ...
        try:
            result = subprocess.run([
                sys.executable, '-m', 'ruff', 'check',
                str(file_path), '--select', 'I001', '--quiet'
            ], capture_output=True, text=True, cwd=self.project_root)
    ...
    def validate_project(self, app_dir: str = "app") -> Dict[str, any]:
        ...
        for py_file in app_path.rglob('*.py'):
            ...
            if file_issue_count > 0:
                ...
                # Update summary
                results['summary']['unused_imports'] += len(file_result['unused_imports'])
                results['summary']['import_errors'] += len(file_result['import_errors'])
                results['summary']['sorting_issues'] += len(file_result['sorting_issues'])
...
def main():
    ...
    parser.add_argument('--app-dir', default='app', help='App directory to validate (default: app)')
    parser.add_argument('--verbose', '-v', action='store_true', help='Show detailed results')
    parser.add_argument('--project-root', default='.', help='Project root directory (default: current)')
