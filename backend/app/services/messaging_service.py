...
class MessagingRulesService:
    ...
    async def can_communicate(self, db: AsyncSession, user1_id: int, user2_id: int) -> Tuple[bool, str]:
        """
        ...
        if (UserRole.COMPANY_ADMIN.value in user1_roles and user1.company_id == user2.company_id) or \
           (UserRole.COMPANY_ADMIN.value in user2_roles and user1.company_id == user2.company_id):
        ...
        allowed_combinations = {
            ...
            (UserRole.RECRUITER.value, UserRole.RECRUITER.value),  # Recruiters can talk to each other
        }
        ...
        if (primary_role1 == UserRole.CANDIDATE.value and primary_role2 == UserRole.EMPLOYER.value) or \
           (primary_role1 == UserRole.EMPLOYER.value and primary_role2 == UserRole.CANDIDATE.value):
            return False, "Direct communication between Candidates and Employers is not allowed. Please communicate through a Recruiter."
        
        return False, f"Communication not allowed between {primary_role1} and {primary_role2}"

    async def can_access_conversation(self, db: AsyncSession, user_id: int, conversation_id: int) -> Tuple[bool, str]:
        ...
        if primary_role == UserRole.SUPER_ADMIN.value:
            ...
            result = await db.execute(
                ...
                .where(
                    ...
                    or_(
                        User.user_roles.any(role_id=select("roles").where("name" == UserRole.CANDIDATE.value)),
                        User.user_roles.any(role_id=select("roles").where("name" == UserRole.EMPLOYER.value)),
                        User.user_roles.any(role_id=select("roles").where("name" == UserRole.RECRUITER.value))
                    )
                )
            )
            potential_participants = result.scalars().all()
...
class MessagingService:
    ...
    async def create_conversation(
        ...
        all_participants = [current_user_id] + [pid for pid in participant_ids if pid != current_user_id]
        
        # For direct conversations (2 participants), check if they can communicate
        if len(all_participants) == 2:
            can_comm, reason = await self.rules_service.can_communicate(db, all_participants[0], all_participants[1])
            if not can_comm:
                raise HTTPException(
        ...
        if len(all_participants) == 2:
            existing_conv = await self._find_existing_direct_conversation(db, all_participants)
            if existing_conv:
        ...
        for participant_id in all_participants:
            # Verify user exists
            user_result = await db.execute(select(User).where(User.id == participant_id))
    ...
    async def send_message(
        ...
        can_access, reason = await self.rules_service.can_access_conversation(db, sender_id, conversation_id)
        if not can_access:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=reason
    ...
    async def get_conversation_messages(
        ...
        # Check access
        can_access, reason = await self.rules_service.can_access_conversation(db, user_id, conversation_id)
        if not can_access:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=reason
    ...
    async def mark_messages_as_read(
        ...
        # Create read records
        read_records = [
            MessageRead(message_id=msg_id, user_id=user_id)
