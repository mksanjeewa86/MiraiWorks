...
class RecordingService:
    ...
    async def start_recording(
        ...
        consents: list[RecordingConsent]
    ) -> Optional[str]:
        ...
        try:
            ...
            if not self._all_participants_consented(consents):
                logger.warning(f"Cannot start recording: Not all participants consented for call {video_call.id}")
                return None

            # Generate recording session
            recording_id = f"rec_{video_call.room_id}_{int(datetime.now(timezone.utc).timestamp())}"
            ...
            if recording_url:
                ...
                logger.info(f"Started recording {recording_id} for video call {video_call.id}")
                return recording_id
    ...
    async def stop_recording(self, video_call: VideoCall) -> Optional[str]:
        ...
        try:
            ...
            if final_url:
                ...
                logger.info(f"Stopped recording {recording_id} for video call {video_call.id}")
                return final_url
    ...
    async def pause_recording(self, video_call: VideoCall) -> bool:
        ...
        try:
            ...
            if success:
                recording_session['paused_at'] = datetime.now(timezone.utc)
    ...
    async def resume_recording(self, video_call: VideoCall) -> bool:
        ...
        try:
            ...
            if success:
                recording_session['resumed_at'] = datetime.now(timezone.utc)
    ...
    async def get_recording_status(self, video_call: VideoCall) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'status': 'active',
                'duration_seconds': (
                    datetime.now(timezone.utc) - recording_session['started_at']
                ).total_seconds()
            }
    ...
    async def validate_recording_consent(
        ...
        try:
            ...
            for participant_id in participants:
                consent = next(
                    (c for c in consents if c.user_id == participant_id), 
                    None
                )
            ...
            return {
                ...
                'missing_consents': [
                    str(pid) for pid in participants 
    ...
    async def generate_recording_metadata(
        ...
        try:
            metadata = {
                ...
                'ended_at': video_call.ended_at.isoformat() if video_call.ended_at else None,
                'participants': {
    ...
    def _all_participants_consented(self, consents: list[RecordingConsent]) -> bool:
        ...
        consented_users = {
            consent.user_id for consent in consents 
