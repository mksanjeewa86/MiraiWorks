...
class InterviewService:
    ...
    async def create_interview(
        ...
        logger.info(f"Created interview {interview.id} between candidate {candidate_id} and employer company {employer_company_id}")
    ...
    async def create_proposal(
        ...
        await self._validate_proposal_permissions(proposer_role, interview.status, proposed_by, interview)
        
        # Check for time conflicts
        await self._check_time_conflicts(db, interview, start_datetime, end_datetime, proposed_by)
        ...
        logger.info(f"Created proposal {proposal.id} for interview {interview_id} by user {proposed_by}")
    ...
    async def respond_to_proposal(
        ...
        proposal_result = await db.execute(
            select(InterviewProposal).options(
                ...
                selectinload(InterviewProposal.interview).selectinload(Interview.employer_company)
    ...
    async def reschedule_interview(
        ...
        # Check for conflicts
        await self._check_time_conflicts(db, interview, new_start, new_end, rescheduled_by)
    ...
    async def get_user_interviews(
        ...
        # Order and paginate
        query = query.order_by(Interview.scheduled_start.desc()).offset(offset).limit(limit)
    ...
    async def _get_proposer_role(self, db: AsyncSession, user_id: int, interview: Interview) -> str:
    ...
    async def _validate_proposal_permissions(
        ...
        if interview_status not in [InterviewStatus.PROPOSED.value, InterviewStatus.ACCEPTED.value]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    async def _check_time_conflicts(
        ...
        if conflicting_interviews:
            logger.warning(f"Time conflict detected for proposal: {len(conflicting_interviews)} conflicts")
    ...
    async def _accept_proposal(self, db: AsyncSession, proposal: InterviewProposal):
        ...
        await db.execute(
            ...
            .values(status="declined", response_notes="Automatically declined - another proposal accepted")
        )
    ...
    async def _create_calendar_events(self, db: AsyncSession, interview: Interview):
        ...
        for participant in participants:
            ...
            for account in calendar_accounts.scalars().all():
                try:
                    await self._create_calendar_event_for_account(db, interview, account)
                except Exception as e:
                    logger.error(f"Failed to create calendar event for user {participant.id}: {e}")
