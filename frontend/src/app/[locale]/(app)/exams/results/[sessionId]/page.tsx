...
import { examTakingApi } from '@/api/exam';
  ...
  const fetchResults = async () => {
    try {
      ...
      if (response.data) {
        ...
        setShowCorrectAnswers(!!response.data.questions);
      }
    } catch (error) {
      router.push(ROUTES.EXAMS.BASE);
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <Button asChild>
            <Link href={ROUTES.EXAMS.BASE}>Back to Exams</Link>
          </Button>
        </div>
      </div>
    );
          ...
          <CardContent>
            <div className="grid gap-4 md:grid-cols-2 text-sm">
              ...
              <div className="flex items-center gap-2">
                <Calendar className="h-4 w-4 text-gray-400" />
                <span>
                  Completed: {new Date(session.completed_at ?? new Date()).toLocaleString()}
                </span>
              </div>
              <div className="flex items-center gap-2">
