...
export const calendarApi = {
  ...
  async updateEvent(id: number, eventData: Partial<CalendarEvent>): Promise<ApiResponse<CalendarEvent>> {
    const response = await apiClient.put<CalendarEvent>(API_ENDPOINTS.CALENDAR.EVENT_BY_ID(id), eventData);
    return { data: response.data, success: true };
  },
  ...
  async getAvailability(userId: number, date: string): Promise<ApiResponse<{
    date: string;
  ...
  async getOutlookAuthUrl(): Promise<ApiResponse<{ auth_url: string }>> {
    const response = await apiClient.get<{ auth_url: string }>('/api/calendar/oauth/microsoft/start');
    return { data: response.data, success: true };
  },

  async deleteConnection(connectionId: number): Promise<ApiResponse<{ message: string }>> {
    const response = await apiClient.delete<{ message: string }>(`/api/calendar/accounts/${connectionId}`);
    return { data: response.data, success: true };
  },

  async syncCalendar(connectionId: number): Promise<ApiResponse<{ message: string }>> {
    const response = await apiClient.post<{ message: string }>(`/api/calendar/accounts/${connectionId}/sync`);
    return { data: response.data, success: true };
  },

  async updateConnection(connectionId: number, updates: Record<string, unknown>): Promise<ApiResponse<CalendarConnection>> {
    const response = await apiClient.put<CalendarConnection>(`/api/calendar/accounts/${connectionId}`, updates);
    return { data: response.data, success: true };
  },
};
...
export const createCalendarEvent = async (eventData: CalendarEventInput): Promise<ApiResponse<CalendarEvent>> => {
  const response = await publicApiClient.post<CalendarEvent>(API_ENDPOINTS.CALENDAR.EVENTS, eventData);
  return { data: response.data, success: true };
};
...
export const updateCalendarEvent = async (eventId: string, eventData: CalendarEventInput): Promise<ApiResponse<CalendarEvent>> => {
  const response = await apiClient.put<CalendarEvent>(API_ENDPOINTS.CALENDAR.EVENT_BY_ID(eventId), eventData);
  return { data: response.data, success: true };
};
