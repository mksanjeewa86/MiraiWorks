...
class TestInterviewService:
    ...
    @pytest.fixture
    async def interview(self, db_session: AsyncSession, candidate_user, recruiter_user, employer_company):
        interview = Interview(
    ...
    async def test_get_interview(
        ...
        retrieved_interview = await interview_service.get_interview(db_session, interview.id)
    ...
    async def test_respond_to_proposal_declined(
        ...
        response_data = ProposalResponse(
            response="declined",
            notes="Time doesn't work for me"
        )
    ...
    async def test_reschedule_interview(
        ...
        rescheduled_interview = await interview_service.reschedule_interview(
            db_session, interview.id, new_start, new_end, "Conflict in schedule", recruiter_user.id
        )
    ...
    async def test_check_interview_conflicts_no_calendar(
        ...
        conflicts = await interview_service.check_interview_conflicts(db_session, interview.id)
    ...
    async def test_check_interview_conflicts_with_calendar(
        ...
        mock_list_events.return_value = {
            "items": [{
                ...
                "start": {"dateTime": (datetime.utcnow() + timedelta(days=1, minutes=30)).isoformat()},
                "end": {"dateTime": (datetime.utcnow() + timedelta(days=1, hours=1, minutes=30)).isoformat()}
            }]
        }
    ...
    async def test_multiple_proposals_for_interview(
        ...
        proposals = await interview_service.get_interview_proposals(db_session, interview.id)
...
class TestInterviewAPI:
    ...
    def test_respond_to_proposal(self, client: TestClient, auth_headers):
        ...
        response_data = {
            "response": "accepted",
            "notes": "This time works perfectly"
        }
    ...
    def test_list_interviews_with_filters(self, client: TestClient, auth_headers):
        ...
        response = client.get("/api/v1/interviews/", params=params, headers=auth_headers)
