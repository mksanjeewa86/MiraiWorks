...
function PositionsPageContent() {
  ...
  const categories = [
    { value: 'all', label: t('filters.categories.allCategories') },
    { value: 'technology', label: t('filters.categories.technology') },
    { value: 'marketing', label: t('filters.categories.marketing') },
    { value: 'design', label: t('filters.categories.design') },
    { value: 'sales', label: t('filters.categories.sales') },
    { value: 'finance', label: t('filters.categories.finance') },
    { value: 'healthcare', label: t('filters.categories.healthcare') },
  ];

  const jobTypes = [
    { value: 'all', label: t('filters.jobTypes.allTypes') },
    { value: 'full_time', label: t('filters.jobTypes.fullTime') },
    { value: 'part_time', label: t('filters.jobTypes.partTime') },
    { value: 'contract', label: t('filters.jobTypes.contract') },
    { value: 'temporary', label: t('filters.jobTypes.temporary') },
    { value: 'internship', label: t('filters.jobTypes.internship') },
    { value: 'freelance', label: t('filters.jobTypes.freelance') },
  ];

  const prefectureOptions = [
    { value: 'all', label: t('filters.location.allLocations') },
    ...PREFECTURES.map((pref) => ({
      value: pref.nameJa,
      label: `${pref.nameEn} (${pref.nameJa})`,
    })),
    { value: 'リモート', label: t('filters.location.remote') },
  ];

  const salaryRanges = [
    { value: 'all', label: t('filters.salary.anySalary') },
  ...
  const dateFilters = [
    { value: 'all', label: t('filters.datePosted.anyTime') },
    { value: '1', label: t('filters.datePosted.last24Hours') },
    { value: '3', label: t('filters.datePosted.last3Days') },
    { value: '7', label: t('filters.datePosted.lastWeek') },
    { value: '30', label: t('filters.datePosted.lastMonth') },
  ];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {(selectedCategory !== 'all' ||
            ...
            <div className="mb-6 p-4 bg-indigo-50 rounded-xl border border-indigo-200">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-sm font-semibold text-indigo-900">{t('filters.activeFilters')}</h3>
                <button
                  onClick={() => {
                    setSearchQuery('');
              ...
              <div className="flex flex-wrap gap-2">
                {searchQuery && (
                  <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                    {t('filters.activeFilterLabels.search')}: &ldquo;{searchQuery}&rdquo;
                    <button
                ...
                {selectedCategory !== 'all' && (
                  <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                    {t('filters.activeFilterLabels.category')}: {categories.find((c) => c.value === selectedCategory)?.label}
                    <button
                ...
                {selectedType !== 'all' && (
                  <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                    {t('filters.activeFilterLabels.type')}: {jobTypes.find((jt) => jt.value === selectedType)?.label}
                    <button
                ...
                {selectedPrefecture !== 'all' && (
                  <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                    {t('filters.activeFilterLabels.location')}: {prefectureOptions.find((p) => p.value === selectedPrefecture)?.label}
                    <button
                ...
                {selectedSalaryRange !== 'all' && (
                  <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                    {t('filters.activeFilterLabels.salary')}: {salaryRanges.find((s) => s.value === selectedSalaryRange)?.label}
                    <button
                ...
                {selectedDateFilter !== 'all' && (
                  <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                    {t('filters.activeFilterLabels.posted')}: {dateFilters.find((d) => d.value === selectedDateFilter)?.label}
                    <button
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">
                {loading
                  ? t('results.loading')
                  : t(totalPositions === 1 ? 'results.positionsFound' : 'results.positionsFoundPlural', { count: totalPositions })}
              </h2>
              {!loading && totalPositions > 0 && (
                <p className="text-gray-600 mt-1">
                  {t('results.showing', { start: startPosition, end: endPosition, total: totalPositions })}
                </p>
              )}
            </div>
            ...
            {!loading && totalPages > 1 && (
              <div className="text-sm text-gray-500">
                {t('results.pageInfo', { current: currentPage, total: totalPages })}
              </div>
            )}
          </div>
          ...
          {loading && (
            <div className="text-center py-16">
              ...
              <p className="text-gray-600">{t('results.loadingPositions')}</p>
            </div>
          )}
          ...
          {error && (
            <div className="text-center py-16">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-2">{t('errors.title')}</h3>
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-indigo-600 font-medium">
                          {job.company_name || t('results.unknownCompany')}
                        </p>
                      </div>
                      {job.is_featured && (
                    ...
                    {job.salary_min && job.salary_max && job.show_salary && (
                      <div className="mb-3">
                        ...
                        <span className="text-xs text-gray-500 ml-1">{t('filters.salary.perYear')}</span>
                      </div>
                    )}
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-100">
                      <span className="text-xs text-gray-500">
                        ...
                        {(() => {
                          ...
                          if (!dateStr) return t('results.recently');
                          try {
                            const date = new Date(dateStr);
                            return isNaN(date.getTime()) ? t('results.recently') : date.toLocaleDateString();
                          } catch {
                            return t('results.recently');
                          }
                        })()}
                      </span>
                    </div>
