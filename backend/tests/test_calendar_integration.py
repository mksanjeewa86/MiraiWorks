...
class TestMicrosoftCalendarService:
    ...
    async def test_create_event_microsoft(self, mock_post, microsoft_service):
        ...
        created_event = await microsoft_service.create_event(
            "access_token_123", event_data
        )
...
class TestCalendarIntegrationAPI:
    ...
    def test_connect_google_calendar(
        self, mock_get_oauth_url, client: TestClient, auth_headers
    ):
        """Test initiating Google Calendar connection."""
        mock_get_oauth_url.return_value = (
    ...
    def test_sync_calendars_no_integration(self, client: TestClient, auth_headers):
        ...
        response = client.post("/api/v1/calendar/sync", json={}, headers=auth_headers)
...
class TestCalendarEvents:
    ...
    async def test_calendar_event_creation(
        self, db_session: AsyncSession, calendar_integration
    ):
    ...
    async def test_get_events_by_date_range(
        self, db_session: AsyncSession, calendar_event
    ):
    ...
    async def test_event_conflict_detection(
        self, db_session: AsyncSession, calendar_integration
    ):
