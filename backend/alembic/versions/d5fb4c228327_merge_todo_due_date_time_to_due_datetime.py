...
def upgrade() -> None:
    ...
    print(f"Current state: due_date={has_due_date}, due_time={has_due_time}, due_datetime={has_due_datetime}")
    ...
    op.add_column('todos', sa.Column('due_datetime', sa.DateTime(timezone=True), nullable=True))
    ...
    # Get total count
    result = connection.execute(sa.text("SELECT COUNT(*) FROM todos WHERE due_date IS NOT NULL"))
