...
async def send_message(
    ...
    from app.routers.messaging_ws import broadcast_new_message
    try:
...
async def attachment_scan_complete(
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Scan failed"
        )
...
async def search_conversation_participants(
    ...
    participants = (
        await messaging_service.rules_service.get_allowed_conversation_participants(
            db, current_user.id
        )
    )
