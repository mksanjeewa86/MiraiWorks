...
def requires_permission(permission: str):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            has_perm = any(
                has_permission(UserRole(role), permission) for role in user_roles
            )
        ...
        if "company_id" in kwargs:
            ...
            if db:
                ...
                result = await db.execute(
                    select(User.company_id).where(User.id == kwargs["user_id"])
                )
                company_id = result.scalar_one_or_none()

        # Check access
        if company_id is not None and not check_company_access(
