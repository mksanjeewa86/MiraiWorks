...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('employer_company_id', sa.Integer(), sa.ForeignKey('companies.id', ondelete='CASCADE'), nullable=False, index=True),
    ...
    op.create_table(
        'process_nodes',
        sa.Column('id', sa.Integer(), primary_key=True, nullable=False),
        sa.Column('process_id', sa.Integer(), sa.ForeignKey('recruitment_processes.id', ondelete='CASCADE'), nullable=False, index=True),
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False, index=True),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.func.now(), onupdate=sa.func.now(), nullable=False),
        sa.UniqueConstraint('process_id', 'sequence_order', name='uq_process_node_sequence')
    )
    ...
    op.create_table(
        ...
        sa.Column('id', sa.Integer(), primary_key=True, nullable=False),
        sa.Column('process_id', sa.Integer(), sa.ForeignKey('recruitment_processes.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('source_node_id', sa.Integer(), sa.ForeignKey('process_nodes.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('target_node_id', sa.Integer(), sa.ForeignKey('process_nodes.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('condition_type', sa.String(50), nullable=False, default='success', index=True),
    ...
    op.create_table(
        'candidate_processes',
        sa.Column('id', sa.Integer(), primary_key=True, nullable=False),
        sa.Column('candidate_id', sa.Integer(), sa.ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('process_id', sa.Integer(), sa.ForeignKey('recruitment_processes.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('current_node_id', sa.Integer(), sa.ForeignKey('process_nodes.id', ondelete='SET NULL'), nullable=True, index=True),
        sa.Column('status', sa.String(50), nullable=False, default='not_started', index=True),
        sa.Column('assigned_recruiter_id', sa.Integer(), sa.ForeignKey('users.id', ondelete='SET NULL'), nullable=True, index=True),
    ...
    op.create_table(
        'node_executions',
        sa.Column('id', sa.Integer(), primary_key=True, nullable=False),
        sa.Column('candidate_process_id', sa.Integer(), sa.ForeignKey('candidate_processes.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('node_id', sa.Integer(), sa.ForeignKey('process_nodes.id', ondelete='CASCADE'), nullable=False, index=True),
        ...
        sa.Column('interview_id', sa.Integer(), sa.ForeignKey('interviews.id', ondelete='SET NULL'), nullable=True, index=True),
        sa.Column('todo_id', sa.Integer(), sa.ForeignKey('todos.id', ondelete='SET NULL'), nullable=True, index=True),
        sa.Column('started_at', sa.DateTime(timezone=True), nullable=True),
        sa.Column('completed_at', sa.DateTime(timezone=True), nullable=True),
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False, index=True),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.func.now(), onupdate=sa.func.now(), nullable=False),
        sa.UniqueConstraint('candidate_process_id', 'node_id', name='uq_candidate_node_execution')
    )
    ...
    op.create_table(
        ...
        sa.Column('added_by', sa.Integer(), sa.ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('added_at', sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False, index=True),
        sa.UniqueConstraint('process_id', 'user_id', name='uq_process_viewer')
    )

    # Create indexes for better performance
    op.create_index('ix_recruitment_processes_status_company', 'recruitment_processes', ['status', 'employer_company_id'])
    op.create_index('ix_process_nodes_process_sequence', 'process_nodes', ['process_id', 'sequence_order'])
    op.create_index('ix_candidate_processes_status_recruiter', 'candidate_processes', ['status', 'assigned_recruiter_id'])
    op.create_index('ix_node_executions_status_assigned', 'node_executions', ['status', 'assigned_to'])
