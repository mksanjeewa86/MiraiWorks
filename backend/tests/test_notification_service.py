...
class TestNotificationService:
    ...
    @pytest.mark.asyncio
    async def test_create_notification_success_with_polling_architecture(
        self,
        db_session: AsyncSession,
        test_user: User,
    ):
        """Test notification creation with polling architecture (no real-time WebSocket)."""
        result = await notification_service.create_notification(
            db_session,
            user_id=test_user.id,
            notification_type="new_message",
        ...
        # With polling architecture, notifications are retrieved via API polling
        # No real-time WebSocket broadcast verification needed

    # WebSocket real-time functionality has been removed
    # Notifications are now delivered via polling on the frontend
