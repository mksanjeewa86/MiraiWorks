...
class ExamQuestionBase(BaseModel):
    ...
    options: Optional[Dict[str, str]] = None  # {"A": "Option 1", "B": "Option 2"}
    correct_answers: Optional[List[str]] = None  # ["A"] or ["A", "B"]
...
class ExamQuestionPublic(BaseModel):
    ...
    is_required: bool
    options: Optional[Dict[str, str]]  # No correct answers exposed
...
class ExamResultSummary(BaseModel):
    ...
    questions: Optional[List[ExamQuestionInfo]] = None  # Include if showing correct answers
    monitoring_events: Optional[List[ExamMonitoringEventInfo]] = None
