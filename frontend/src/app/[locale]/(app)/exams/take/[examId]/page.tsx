...
export default function TakeExamPage() {
  ...
  const submitExam = async () => {
    ...
    try {
      ...
      if (testMode) {
        router.push(ROUTES.ADMIN.EXAMS.PREVIEW(examData.session.exam_id));
      } else {
        router.push(ROUTES.EXAMS.RESULTS(examData.session.id));
      }
    } catch (error) {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <Button onClick={() => router.push(ROUTES.EXAMS.BASE)}>Back to Exams</Button>
        </div>
      </div>
    );
  }
