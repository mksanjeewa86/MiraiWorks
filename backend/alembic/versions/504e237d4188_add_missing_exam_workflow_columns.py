...
def upgrade() -> None:
    ...
    op.alter_column('question_banks', 'created_by',
    ...
    op.create_foreign_key('fk_exam_questions_source_bank',
                         'exam_questions', 'question_banks',
                         ['source_bank_id'], ['id'],
                         ondelete='SET NULL')
    op.create_foreign_key('fk_exam_questions_source_question',
    ...
    op.create_index('ix_exam_questions_source_bank_id', 'exam_questions', ['source_bank_id'])
    ...
    op.add_column('todos',
                  sa.Column('exam_assignment_id', sa.Integer(), nullable=True))
    op.add_column('todos',
                  sa.Column('exam_config', sa.JSON(), nullable=True))

    # Create foreign keys for todos
    op.create_foreign_key('fk_todos_exam',
                         'todos', 'exams',
                         ['exam_id'], ['id'],
                         ondelete='SET NULL')
    op.create_foreign_key('fk_todos_exam_assignment',
                         'todos', 'exam_assignments',
                         ['exam_assignment_id'], ['id'],
                         ondelete='SET NULL')
    ...
    op.add_column('exam_assignments',
    ...
    op.create_foreign_key('fk_exam_assignments_todo',
                         'exam_assignments', 'todos',
                         ['todo_id'], ['id'],
                         ondelete='SET NULL')
    op.create_foreign_key('fk_exam_assignments_workflow_execution',
                         'exam_assignments', 'workflow_node_executions',
                         ['workflow_node_execution_id'], ['id'],
                         ondelete='SET NULL')

    # Create indexes for exam_assignments
    op.create_index('ix_exam_assignments_todo_id', 'exam_assignments', ['todo_id'])
    op.create_index('ix_exam_assignments_workflow_node_execution_id',
                   'exam_assignments', ['workflow_node_execution_id'])
...
def downgrade() -> None:
    ...
    op.drop_constraint('fk_exam_assignments_workflow_execution', 'exam_assignments', type_='foreignkey')
    op.drop_constraint('fk_exam_assignments_todo', 'exam_assignments', type_='foreignkey')
    ...
    op.drop_constraint('fk_exam_questions_source_question', 'exam_questions', type_='foreignkey')
    op.drop_constraint('fk_exam_questions_source_bank', 'exam_questions', type_='foreignkey')
    ...
    op.alter_column('question_banks', 'created_by_id',
