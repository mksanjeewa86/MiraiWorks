...
class AntivirusService:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"Failed to download file from S3: {response.status_code}")
                    return None
    ...
    async def scan_attachment(self, db: AsyncSession, attachment_id: int) -> bool:
        ...
        try:
            ...
            if attachment.virus_status in [VirusStatus.CLEAN.value, VirusStatus.INFECTED.value]:
                logger.info(f"Attachment {attachment_id} already scanned: {attachment.virus_status}")
                return True
            ...
            if status == VirusStatus.INFECTED:
                logger.warning(f"SECURITY: Infected file detected - Attachment ID: {attachment_id}, "
