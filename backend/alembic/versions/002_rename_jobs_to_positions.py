...
def upgrade() -> None:
    ...
    op.drop_constraint(
        "position_applications_ibfk_1", "position_applications", type_="foreignkey"
    )
    ...
    op.create_index(
        "idx_positions_company_status", "positions", ["company_id", "status"]
    )
    op.create_index("idx_positions_published", "positions", ["published_at", "status"])
    op.create_index(
        "idx_positions_location_type", "positions", ["country", "city", "job_type"]
    )
    op.create_index(
...
def downgrade() -> None:
    ...
    op.drop_index(
        "idx_applications_position_status", table_name="position_applications"
    )
    op.drop_index("idx_applications_candidate", table_name="position_applications")
    op.drop_index("idx_applications_status_date", table_name="position_applications")
    ...
    op.create_index(
        "idx_jobs_experience_remote", "jobs", ["experience_level", "remote_type"]
    )
    op.create_index(
        "idx_jobs_featured_status", "jobs", ["is_featured", "status", "published_at"]
    )

    # Recreate job_applications indexes
    op.create_index(
        "idx_applications_job_status", "job_applications", ["job_id", "status"]
    )
    op.create_index(
        "idx_applications_candidate", "job_applications", ["candidate_id", "applied_at"]
    )
    op.create_index(
        "idx_applications_status_date",
        "job_applications",
