...
class Attachment(Base):
    ...
    sha256_hash = Column(String(64), nullable=False, index=True)  # File hash for deduplication
    
    # Virus scanning
    virus_status = Column(String(20), nullable=False, default=VirusStatus.PENDING.value, index=True)
    virus_scan_result = Column(Text, nullable=True)  # Detailed scan result
    scanned_at = Column(DateTime(timezone=True), nullable=True)
    
    # Metadata
    is_available = Column(Boolean, nullable=False, default=False, index=True)  # Only true after clean scan
