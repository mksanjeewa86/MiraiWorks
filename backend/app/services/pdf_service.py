...
class PDFService:
    ...
    async def generate_pdf(
        ...
        try:
            ...
            logger.info(f"Generated PDF for resume {resume.id}, size: {len(pdf_data)} bytes")
            return result
    ...
    async def _convert_html_to_pdf(self, html_content: str, format: str = "A4") -> bytes:
        ...
        try:
            ...
            # Option 2: Using weasyprint (fallback)
                ...
                raise RuntimeError("No PDF generation library available (playwright or weasyprint)")
    ...
    async def get_pdf_preview_image(self, resume: Resume) -> Optional[str]:
        ...
        try:
            ...
            if preview_data:
                ...
                preview_filename = f"resume_preview_{resume.id}_{uuid.uuid4().hex[:8]}.png"
    ...
    async def bulk_generate_pdfs(self, resume_ids: list[int], user_id: int) -> Dict[str, Any]:
