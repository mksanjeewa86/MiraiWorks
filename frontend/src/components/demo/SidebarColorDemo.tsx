      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            ...
            <div className={`w-64 h-96 ${colorScheme.background} ${colorScheme.border} border rounded-lg overflow-hidden`}>
              {/* Header */}
              ...
              <div className="p-4 space-y-3">
                {/* Active Item */}
                <div className={`px-4 py-2 h-10 rounded-xl ${colorScheme.buttonActive} ${colorScheme.textPrimary} shadow-lg flex items-center`}>
                  <div className={`p-2 rounded-lg bg-white/20 mr-3`}>
                    <div className="w-4 h-4 bg-white rounded"></div>
                  </div>
                  <span className="text-sm font-semibold">Dashboard</span>
                  <div className="ml-auto">
                    <div className={`w-2 h-2 rounded-full ${colorScheme.activeIndicator} shadow-lg animate-pulse`}></div>
                  </div>
                </div>

                {/* Inactive Items */}
                <div className={`px-4 py-2 h-10 rounded-xl ${colorScheme.textSecondary} border ${colorScheme.buttonBorder} flex items-center`}>
              ...
              {/* User Info */}
              <div className={`absolute bottom-0 left-0 right-0 p-4 border-t ${colorScheme.border} ${colorScheme.headerBackground}`}>
                <div className={`flex items-center space-x-3 p-3 rounded-xl ${colorScheme.headerBackground} border ${colorScheme.buttonBorder}`}>
                  <div className={`w-10 h-10 rounded-full ${colorScheme.avatarBackground} flex items-center justify-center ${colorScheme.textPrimary} text-sm font-bold`}>
                    U
                  </div>
                  ...
                  <div className={`w-2 h-2 rounded-full ${colorScheme.statusIndicator} shadow-lg animate-pulse`}></div>
                </div>
              </div>
            </div>
          </div>
            ...
            {roles.map((role) => {
              ...
              return (
                <div key={role} className="text-center">
                  <div className={`w-full h-32 ${scheme.background} ${scheme.border} border rounded-lg mb-2 relative overflow-hidden`}>
                    <div className={`absolute inset-0 ${scheme.backgroundOverlay}`}></div>
                    <div className={`absolute top-2 left-2 w-6 h-6 rounded ${scheme.brandBackground}`}></div>
                    <div className={`absolute bottom-2 left-2 right-2 h-6 ${scheme.buttonActive} rounded`}></div>
                  </div>
