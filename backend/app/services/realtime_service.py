...
class RealtimeService:
            ...
            async for message in self.pubsub.listen():
                if message["type"] == "message":
                    try:
                        ...
                        await self._handle_redis_message(channel, payload, message_handler)
    ...
    async def _handle_redis_message(self, channel: str, payload: Dict[str, Any], handler):
    ...
    async def set_user_online_status(self, user_id: int, is_online: bool, ttl: int = 300):
    ...
    async def get_conversation_state(self, conversation_id: int) -> Optional[Dict[str, Any]]:
...
class RedisMessageHandler:
    ...
    async def handle_new_message(self, conversation_id: int, message_data: Dict[str, Any]):
    ...
    async def handle_user_status_change(
        ...
        try:
            ...
            for connected_user_id in self.connection_manager.get_online_users():
                await self.connection_manager.send_to_user(connected_user_id, ws_message)
                
        except Exception as e:
            logger.error(f"Error handling user status change broadcast: {e}")
    
    async def handle_notification(self, target_users: List[int], notification_data: Dict[str, Any]):
