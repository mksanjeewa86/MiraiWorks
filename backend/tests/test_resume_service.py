...
from app.models.resume import (
    Resume, WorkExperience, Education, Skill, Project, 
    Certification, Language, Reference, ResumeTemplate, ResumeShare
)
...
from app.schemas.resume import (
    ...
    SkillCreate, ProjectCreate, ShareLinkCreate
)
...
class TestResumeService:
    ...
    async def test_create_resume(
        ...
        resume = await resume_service.create_resume(db_session, resume_data, test_user.id)
    ...
    async def test_get_resume_unauthorized(
        ...
        retrieved_resume = await resume_service.get_resume(
            db_session, sample_resume.id, 99999  # Wrong user ID
        )
    ...
    async def test_add_work_experience(
        ...
        exp_data = WorkExperienceCreate(
            ...
            achievements=["Improved search performance by 40%", "Led team of 5 engineers"],
    ...
    async def test_slug_generation(
        ...
        # Create first resume
        resume_data1 = ResumeCreate(
            title="Software Engineer",
            full_name="Test User"
        )
        resume1 = await resume_service.create_resume(db_session, resume_data1, test_user.id)
        
        # Create second resume with same title
        resume_data2 = ResumeCreate(
            title="Software Engineer",
            full_name="Test User"
        )
        resume2 = await resume_service.create_resume(db_session, resume_data2, test_user.id)
...
class TestTemplateService:
    ...
    async def test_render_with_different_templates(
        ...
        for template_name in templates:
            ...
            assert html_content is not None
            assert "John Doe" in html_content
            assert f"{template_name}-template" in html_content or template_name in html_content
...
class TestResumeParser:
    ...
    async def test_file_validation(self, resume_parser: ResumeParser):
        ...
        is_valid = await resume_parser.validate_resume_file(
            "resume.pdf", 1024 * 1024, "application/pdf"  # 1MB PDF
        )
        ...
        with pytest.raises(ValueError, match="File size too large"):
            await resume_parser.validate_resume_file(
                "resume.pdf", 20 * 1024 * 1024, "application/pdf"  # 20MB
            )
...
class TestResumeAPI:
    ...
    def test_get_templates_api(self, client: TestClient, auth_headers):
        """Test getting available templates."""
        response = client.get("/api/v1/resumes/templates/available", headers=auth_headers)
