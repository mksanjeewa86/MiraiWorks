...
class ResumeParser:
    ...
    async def parse_resume(
        ...
        try:
            ...
            text_content = await self._extract_text(
                file_content, file_extension, content_type
            )
    ...
    async def _extract_text(
        self, file_content: bytes, file_extension: str, content_type: str
    ) -> str:
    ...
    def _extract_personal_info(self, text: str) -> dict[str, Any]:
        ...
        for url in urls:
            if "linkedin.com" in url.lower():
                ...
                info["github_url"] = url
            elif not info.get("website") and not any(
                domain in url.lower()
    ...
    def _extract_work_experience(self, text: str) -> list[dict[str, Any]]:
        ...
        exp_section = self._extract_section(
            text, ["experience", "work experience", "employment", "career"]
        )
    ...
    def _extract_projects(self, text: str) -> list[dict[str, Any]]:
        ...
        projects_section = self._extract_section(
            text, ["projects", "personal projects", "notable projects"]
        )

        if projects_section:
            # Look for GitHub URLs as project indicators
            github_urls = re.findall(
                r"github\.com/[^\s]+", projects_section, re.IGNORECASE
            )
    ...
    def _extract_languages(self, text: str) -> list[dict[str, Any]]:
        ...
        lang_section = self._extract_section(
            text, ["languages", "language skills", "spoken languages"]
        )
        ...
        if lang_section:
            ...
            for language in common_languages:
                if re.search(
                    r"\b" + re.escape(language) + r"\b", lang_section, re.IGNORECASE
                ):
                    # Try to find proficiency level
                    proficiency = "Fluent"  # Default
                    for level_key, level_value in proficiency_levels.items():
                        if re.search(
                            rf"\b{language}\b.*?\b{level_key}\b|\b{level_key}\b.*?\b{language}\b",
                            lang_section,
                            re.IGNORECASE,
                        ):
                    ...
                    languages.append({"name": language, "proficiency": proficiency})
    ...
    async def validate_resume_file(
        ...
        if content_type not in allowed_content_types.get(file_extension, []):
            logger.warning(
                f"Content type mismatch: {content_type} for {file_extension}"
            )
