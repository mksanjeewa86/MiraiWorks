...
class WebRTCConnectionManager:
    def __init__(self):
        self.active_connections: Dict[str, Dict[int, WebSocket]] = {}  # room_id -> {user_id: websocket}
    ...
    async def send_to_room_user(self, room_id: str, target_user_id: int, message: dict):
        ...
        if room_id in self.active_connections and target_user_id in self.active_connections[room_id]:
            try:
                ...
                logger.error(f"Error sending message to user {target_user_id} in room {room_id}: {e}")
...
async def websocket_meeting_endpoint(
    ...
    try:
        ...
        if not meeting.can_join:
            await websocket.close(code=status.WS_1008_POLICY_VIOLATION, reason="Meeting cannot be joined")
            return
        ...
        await webrtc_manager.send_to_user(user_id, {
            "type": "meeting_joined",
            "data": {
                "room_id": room_id,
                "meeting": {
            ...
            if user and room_id:
                ...
                try:
                    ...
                    if meeting:
                        ...
                        active_participants = webrtc_manager.get_room_participants(room_id)
                    ...
                    logger.error(f"Error updating participant status on disconnect: {e}")
...
async def handle_webrtc_message(
    ...
    logger.debug(f"Handling message type '{message_type}' from user {sender_id} in room {room_id}")
    
    if message_type == "offer":
        # WebRTC offer - forward to target peer
        if target_user_id:
            await webrtc_manager.send_to_room_user(room_id, target_user_id, {
                "type": "offer",
                "data": data,
                "sender_id": sender_id
            })
        ...
        if target_user_id:
            ...
            await webrtc_manager.broadcast_to_room(room_id, {
                "type": "ice-candidate",
                "data": data,
                "sender_id": sender_id
    ...
    if meeting.recording_consent_required:
        ...
        for p in participants:
            if p.recording_consent is None:
                await webrtc_manager.send_to_user(user_id, {
                    "type": "error",
                    "data": {"message": "Recording consent required from all participants"}
                })
                return
            elif p.recording_consent is False:
                await webrtc_manager.send_to_user(user_id, {
                    "type": "error",
                    "data": {"message": "Recording consent denied by participant(s)"}
                })
                return
