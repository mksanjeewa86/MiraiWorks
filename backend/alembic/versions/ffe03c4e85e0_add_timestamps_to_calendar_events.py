...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE calendar_events MODIFY COLUMN created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP"
    )
    op.execute(
        "ALTER TABLE calendar_events MODIFY COLUMN updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP"
    )
