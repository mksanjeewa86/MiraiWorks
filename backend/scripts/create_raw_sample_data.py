    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            techcorp_result = await db.execute(
                text("SELECT id FROM companies WHERE domain = 'techcorp.com'")
            )
            techcorp_id = techcorp_result.scalar()

            recruiter_result = await db.execute(
                text("SELECT id FROM companies WHERE domain = 'globalrecruiters.com'")
            )
...
async def main():
    ...
    if not await test_connection():
        ...
        print(
            "2. Database exists: docker exec hrms_db mysql -u hrms -phrms -e 'SHOW DATABASES;'"
        )
        print(
            "3. Try creating database: docker exec hrms_db mysql -u root -proot_password -e 'CREATE DATABASE miraiworks;'"
        )
        return
