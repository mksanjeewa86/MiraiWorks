...
def upgrade() -> None:
    ...
    op.add_column('interviews', sa.Column('is_deleted', sa.Boolean(), nullable=False, server_default=sa.false()))
    op.add_column('interviews', sa.Column('deleted_at', sa.DateTime(timezone=True), nullable=True))
    op.create_index('ix_interviews_is_deleted', 'interviews', ['is_deleted'])

    # Add soft delete columns to recruitment_processes table
    op.add_column('recruitment_processes', sa.Column('is_deleted', sa.Boolean(), nullable=False, server_default=sa.false()))
    op.add_column('recruitment_processes', sa.Column('deleted_at', sa.DateTime(timezone=True), nullable=True))
    op.create_index('ix_recruitment_processes_is_deleted', 'recruitment_processes', ['is_deleted'])
