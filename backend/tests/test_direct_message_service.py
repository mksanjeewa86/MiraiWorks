...
class TestDirectMessageService:
    ...
    async def test_get_conversations(
        ...
        assert (
            conv_with_user2.other_user_name
    ...
    async def test_search_messages(
        ...
        search_request = MessageSearchRequest(query="message 3", limit=10, offset=0)
    ...
    async def test_search_messages_with_user_filter(
        ...
        search_request = MessageSearchRequest(
            query="message", with_user_id=test_user2.id, limit=10, offset=0
        )
    ...
    async def test_mark_messages_as_read(
        ...
        stmt = select(DirectMessage).where(DirectMessage.id.in_(message_ids))
    ...
    @pytest.mark.asyncio
    async def test_empty_search_query(self, db_session: AsyncSession, test_user: User):
        """Test search with empty query."""
        search_request = MessageSearchRequest(query="", limit=10, offset=0)
    ...
    async def test_search_pagination(
        ...
        search_request = MessageSearchRequest(query="message", limit=2, offset=0)
    ...
    async def test_case_insensitive_search(
        ...
        for search_term in search_cases:
            search_request = MessageSearchRequest(query=search_term, limit=10, offset=0)
