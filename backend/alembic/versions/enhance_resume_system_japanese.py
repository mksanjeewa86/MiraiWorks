...
def upgrade() -> None:
    ...
    op.add_column(
        ...
        sa.Column(
            "resume_language",
            sa.Enum("JAPANESE", "ENGLISH", "BILINGUAL", name="resumelanguage"),
            nullable=True,
        ),
    )
    op.add_column(
        "resumes", sa.Column("furigana_name", sa.String(length=100), nullable=True)
    )
    ...
    op.add_column(
        "resumes", sa.Column("nationality", sa.String(length=50), nullable=True)
    )
    op.add_column(
        "resumes", sa.Column("marital_status", sa.String(length=20), nullable=True)
    )
    op.add_column("resumes", sa.Column("emergency_contact", sa.JSON(), nullable=True))
    op.add_column(
        "resumes", sa.Column("photo_path", sa.String(length=500), nullable=True)
    )
    op.add_column("resumes", sa.Column("is_public", sa.Boolean(), nullable=True))
    op.add_column(
        "resumes", sa.Column("public_url_slug", sa.String(length=100), nullable=True)
    )
    ...
    # Create indexes
    op.create_index(
        op.f("ix_resumes_public_url_slug"), "resumes", ["public_url_slug"], unique=True
    )

    # Create resume_message_attachments table
    op.create_table(
        "resume_message_attachments",
    ...
    op.create_index(
        op.f("ix_resume_message_attachments_id"),
        "resume_message_attachments",
        ["id"],
        unique=False,
    )
    ...
    op.execute(
        "UPDATE resumes SET resume_format = 'INTERNATIONAL' WHERE resume_format IS NULL"
    )
    op.execute(
        "UPDATE resumes SET resume_language = 'ENGLISH' WHERE resume_language IS NULL"
    )
    op.execute("UPDATE resumes SET is_public = false WHERE is_public IS NULL")
    op.execute(
        "UPDATE resumes SET can_download_pdf = true WHERE can_download_pdf IS NULL"
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_resume_message_attachments_id"),
        table_name="resume_message_attachments",
    )
