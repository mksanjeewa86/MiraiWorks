...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "viewer_user_agent", sa.Text(), nullable=True, comment="Browser/device info"
        ),
        sa.Column(
            "view_duration",
            sa.Integer(),
        ...
        sa.Column(
            ...
            nullable=True,
            comment="How they found the profile",
        ),
        ...
        sa.ForeignKeyConstraint(
            ["viewer_company_id"], ["companies.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        op.f("ix_profile_views_profile_user_id"),
        "profile_views",
        ["profile_user_id"],
        unique=False,
    )
